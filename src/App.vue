...
//region computed
const snackbarText: Ref<string> = computed(() => mainStore.getMessage);
const progress: Ref<number> = computed(() => mainStore.getProgress);
const loading: Ref<boolean> = computed({
	get: () => mainStore.getLoading,
	set: (v) => mainStore.setLoading(v)
});
const themeDark: Ref<boolean> = computed({
	get: () => mainStore.getThemeDark,
	set: v => mainStore.setThemeDark(v)
});
const error: ComputedRef<boolean> = computed(() => mainStore.getError);
//endregion
